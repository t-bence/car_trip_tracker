...
### One-time manual setup after the first `bundle deploy`

The Lakebase database resource can't be declared in the bundle yet, so attach
it once via the UI: open the app in the Databricks workspace → Configure →
**+ Add resource** → **Lakebase database**, with resource key `database`. This
auto-injects the `PGHOST`/`PGDATABASE`/`PGUSER`/`PGPASSWORD`/`PGPORT` env vars
the app expects, and redeploy the app afterwards for them to take effect.
